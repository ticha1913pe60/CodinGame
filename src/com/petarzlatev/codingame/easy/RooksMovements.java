...
class RooksMovements {
	...
	public static void main(String[] args) {
		...
		for (int i = rook.x - 1; i >= 0; i--) {
			if (horizontalCheck(rook, board, points, i))
				break;
		}
		...
		for (int j = rook.y - 1; j >= 0; j--) {
			if (verticalCheck(rook, board, points, j))
				break;
		}
		...
		points.stream().sorted(Comparator.comparing(p -> p.getString(rook)))
				.forEach(p -> System.out.println(p.getString(rook)));
	}

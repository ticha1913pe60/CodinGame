...
class SimpleAwale {
	...
	public static void main(String[] args) {
		try (Scanner in = new Scanner(System.in)) {
			int[] opBowls = Arrays.stream(in.nextLine().split(" ")).mapToInt(Integer::parseInt).toArray();
			int[] myBowls = Arrays.stream(in.nextLine().split(" ")).mapToInt(Integer::parseInt).toArray();
			int num = in.nextInt();
